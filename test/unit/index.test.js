...
describe('FacebookTokenStrategy:init', () => {
  ...
  it('Should use the default fb graph version when no explicit version is specified', () => {
    let strategy = new FacebookTokenStrategy(STRATEGY_CONFIG, BLANK_FUNCTION);
    assert.equal(strategy._fbGraphVersion, 'v3.2');
    assert.equal(strategy._oauth2._accessTokenUrl,'https://graph.facebook.com/v3.2/oauth/access_token');
    assert.equal(strategy._oauth2._authorizeUrl,'https://www.facebook.com/v3.2/dialog/oauth');
    assert.equal(strategy._profileURL,'https://graph.facebook.com/v3.2/me');
  });
...
describe('FacebookTokenStrategy:userProfile', () => {
  it('Should properly fetch profile', done => {
    ...
    strategy.userProfile('accessToken', (error, profile) => {
      ...
      assert.equal(profile.photos[0].value, 'https://graph.facebook.com/v3.2/794955667239296/picture?type=large');
  ...
  it('Should properly make request with enableProof', done => {
    ...
    strategy.userProfile('accessToken', (error, profile) => {
      assert.equal(strategy._oauth2.get.getCall(0).args[0], 'https://graph.facebook.com/v3.2/me?appsecret_proof=8c340bd01643ab69939ca971314d7a3d64bfb18946cdde566f12fdbf6707d182&fields=id,name,last_name,first_name,middle_name,email');
      strategy._oauth2.get.restore();
      done();
    });
  });
  ...
  it('Should properly make request with profileFields', done => {
    ...
    strategy.userProfile('accessToken', (error, profile) => {
      assert.equal(strategy._oauth2.get.getCall(0).args[0], 'https://graph.facebook.com/v3.2/me?appsecret_proof=8c340bd01643ab69939ca971314d7a3d64bfb18946cdde566f12fdbf6707d182&fields=last_name,first_name,middle_name,custom');
      strategy._oauth2.get.restore();
      done();
    });
  });
  ...
  it('Should use the proper profile image link with profileImage', done => {
    ...
    strategy.userProfile('accessToken', (error, profile) => {
      ...
      assert.equal(profile.photos[0].value, 'https://graph.facebook.com/v3.2/794955667239296/picture?width=1520&height=1520');
